...
class SpreadsheetUploadService:
    ...
    def async_upload(self, token: str, request_file: FileStorage, params: dict):
        ...
        if is_update:
            ...
            thread = threading.Thread(target=self.upload_updates,
                                      args=(submission_url, path))
        else:
            ...
            thread = threading.Thread(target=self.upload,
                                      args=(submission_url, path, project_uuid, update_project))

        thread.start()

        return submission_resource

    def _store_spreadsheet_updates(self, filename: str,
        ...
        path = self.storage_service.store_binary_file(submission_directory,
    ...
    def upload_updates(self, submission_url, path):
        ...
        _LOGGER.info(f'Spreadsheet upload done for {submission_uuid}!')
